...
const getSessionAnalytics = async (req, res) => {
    try {
        ...
        const questionAnalytics = await Promise.all(questions.map(async (q) => {
            ...
            let optionBreakdown = null;
            if (q.type === "mcq" && q.options.length > 0) {
                // Calculate counts and percentages for each option
                const totalResponses = responses.length;
                optionBreakdown = q.options.map((option, index) => {
                    const count = responses.filter(r => r.selectedOpt === index.toString() || r.selectedOpt === option).length;
        ...
        const totalFeedbacks = await prisma_1.default.feedback.count({ where: { sessionId } });
        return res.json({
            stats: {
